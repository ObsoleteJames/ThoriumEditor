...
int GenerateCMakeProject(const FCompileConfig& config)
{
	...
	stream <<	"find_package(Qt6 REQUIRED COMPONENTS Widgets)\n\n"
	...
	if (!bIsExe)
	{
		...
		stream << "qt_add_executable(" << cmakeLib.c_str() << " WIN32 ${Files})\n\n";
	...
	if (dep)
	{
		for (auto depend : *dep)
		{
			stream << "target_link_libraries(" << cmakeLib.c_str() << " PRIVATE ";
